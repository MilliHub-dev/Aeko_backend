...
export type SocketObserver = (parityCase: SocketParityCase) => Promise<RuntimeObservation>;
...
export async function runSocketParity(
  ...
  const [legacy, nest] = await Promise.all([observeLegacy(parityCase), observeNest(parityCase)]);
