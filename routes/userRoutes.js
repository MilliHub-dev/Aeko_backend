...
router.get(
  ...
  privacyFilterMiddleware.checkProfileAccess,
  async (req, res) => {
    try {
      const user = await prisma.user.findUnique({
        where: { id: req.params.id },
        include: {
          posts_posts_userIdTousers: {
            take: 5,
...
router.get("/:id/followers", authMiddleware, async (req, res) => {
  try {
    ...
    const canInteract = await BlockingService.enforceBlockingRules(
      viewerId,
      id,
    );
    if (!canInteract) {
    ...
    if (viewerId === id) {
      ...
      if (!isPrivate) {
        ...
        const followersList = Array.isArray(user.followers)
          ? user.followers
          : [];
    ...
    for (const follower of followers) {
      const canSee = await BlockingService.enforceBlockingRules(
        viewerId,
        follower.id,
      );
      if (canSee) {
