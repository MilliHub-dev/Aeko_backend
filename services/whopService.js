 ...
 *
 * Whop's "Option 2: embedded checkout" with dynamic pricing. Each checkout is
 * created with an INLINE `one_time` plan, so the price comes from our own
 * SubscriptionPlan row at request time and no product has to exist in Whop.
 *
 * Why one_time rather than renewal: Whop rejects an inline `renewal` plan
 * without a product ("In order to create a renewal plan, you must pass in
 * product details"). Each billing period is charged as a one-time payment,
 * and the period itself is tracked by our backend, which sets the expiry from
 * the plan's duration when the payment succeeds.
 *
 * The embed component (WhopCheckoutEmbed) is web-only and cannot run in this
 * React Native app. The same checkout configuration returns `purchase_url`,
 * the Whop-hosted page for that exact session, which the app opens instead.
 *
...
export const createCheckout = async ({
  ...
  // Exactly the documented Option 2 shape: the account, an inline one-time
  // plan priced now, and metadata. Whop copies metadata onto the payment,
  // which is how the webhook maps the payment back to our transaction.
  const payload = {
